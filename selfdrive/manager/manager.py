...
def manager_init() -> None:
  ...
  if slc_priority != 0:
    ...
    priority_list = priorities_mapping.get(slc_priority, ["None", "None", "None"])
    for i, priority in enumerate(priority_list, start=1):
      params.put(f"SLCPriority{i}", priority)
  ...
  for priority_key in ["SLCPriority1", "SLCPriority2", "SLCPriority3"]:
    priority_value = params.get(priority_key)
    if priority_value.isdigit():
      if priority_value in priorities_mapping and priority_value != 0:
        params.put(priority_key, priorities_mapping[priority_value])
